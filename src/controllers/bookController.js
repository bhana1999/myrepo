...
const getbook = async function (req , res){
    ...
    let books = await BookModel.find({year:2000})
    res.send({msg:books})
}
...
const indianprice = async function (req , res){
    //let data = req.body
    console.log(req)
    let books = await BookModel.find({indianPrice:{$nin:[100,200,500]}})
    res.send({msg:books})
}

const getRmBooks = async function(req , res){
...
/**const getBooklist= async function (req, res) {
    ...
    //n // SELECT keys that we want
   // { authorName : "Chetan Bhagat" , isPublished: true  } ) // AND
